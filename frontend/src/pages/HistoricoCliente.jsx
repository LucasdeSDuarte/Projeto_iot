...
export default function HistoricoCliente() {
  ...
  return (
    <DashboardLayout tipo="cliente">
      <h1 className="text-2xl font-bold mb-4 text-zinc-800 dark:text-white">Histórico de Leituras</h1>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <select
          onChange={(e) => setFiltroAlarme(e.target.value)}
          className="p-2 rounded border dark:bg-zinc-800 dark:border-zinc-600 dark:text-white"
        >
          ...
          <tbody>
            {dadosFiltrados.map((item, idx) => (
              <tr key={idx} className="border-b border-gray-200 dark:border-zinc-600">
                <td className="py-2 px-4 text-zinc-800 dark:text-white">{item.torre_id}</td>
