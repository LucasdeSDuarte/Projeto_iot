...
export default function HistoricoColaborador() {
  ...
  return (
    <DashboardLayout tipo="colaborador">
      <h1 className="text-2xl font-bold mb-4">Histórico Geral de Leituras</h1>

      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <select onChange={(e) => setFiltroCliente(e.target.value)} className="p-2 rounded border">
          ...
          <tbody>
            {dadosFiltrados.map((item, idx) => (
              <tr key={idx} className="border-b">
                <td className="py-2 px-4">{item.cliente_id}</td>
            ...
            {dadosFiltrados.length === 0 && (
              <tr>
                <td colSpan="6" className="py-4 text-center text-gray-500">Nenhum dado encontrado</td>
              </tr>
            )}
          </tbody>
        </table>
